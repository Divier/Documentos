...
(function () {
    ...
    moment.fn.toMomentFormatString = function (formatString) {
      if (!javaDateFormats[formatString]) {
    	  ...
    	  while(formatString.indexOf("'") != -1) { 
    	  	if((i%2)==0) {
    	  		formatString = formatString.replaceAt(formatString.indexOf("'"), "[");
    	  	} else {
    	  		formatString = formatString.replaceAt(formatString.indexOf("'"), "]");
    	  	} 
    	  	i++;
  ...
  var translateFormat = function (formatString, mapping) {
    var len = formatString.length,
        ...
        currentChar = "",
        resultString = "";
    var isEscapeText = false;
    for (; i < len; i++) {
      currentChar = formatString.charAt(i);
      if((currentChar == "'" || currentChar == "]") && isEscapeText) {
    	  isEscapeText = false; 
      } else if(currentChar == "'" || currentChar == "[") {
    	  isEscapeText = true; 
      }
      if ((lastChar === null || lastChar !== currentChar) && !isEscapeText) {
        // change detected
