...
public class AdminCuentaUsuarioMB extends AbstractManagedBean {
    ...
    /**
     * Interfaz del EJB ParametrosSeguridadEJB necesario para consultar los parametros relacionados con las politicas de la contrase�a
     */
    @EJB
    ...
    /**
     * Consulta las politicas de seguridad parametrizadas para la contrase�a y las agrega a una lista para ser mostradas en la interfaz cuando se va a
    ...
    public String modificarPassword() {
        ...
        try {
            // Valida que la Nueva contrase�a y la Confirmacion de la nueva contrase�a sean exactamente iguales(FA3)
            if (!pwNuevo.equals(confirmacionPwNuevo)) {
                addErrorMessage(NOMBRE_BUNDLE_LOGIN, "mensaje_confirmacion_pw");
            } else {
                login = getUsuarioDto().getLogin();
    ...
    public void cargarDatosUsuario() {
        ...
        if (!datosCargados) {
            ...
            ldap = getUsuarioDto().isAutenticacionConLdap();
            datosCargados = true;
        }
    }
